...
void ButtonInit(void)
{
    ...
    GPIOPinTypeADC(GPIO_PORTE_BASE, GPIO_PIN_0); //?? GPIO setup for analog input AIN3 //Pin Summary Pin: #23  Function: analog  Instance: AIN3  GPIO: PE_0
...
//    static bool tic = false;
    ...
    //>>>>>>>>>right
    if (presses & (1<<5)) { // EK-TM4C1294XL button 1 pressed
        running = !running; //halt the timer
        //----check button and store ID-----
        fifo_put('D');
        isPutFlag = 0;
    }

    //<<<<<<<<<<left
    else if (presses & (1<<6) ){ // EK-TM4C1294XL button 2 pressed
        fifo_put ('A'); //This is working
        isPutFlag = 0;
    }
    //^^^^^^^^^^^^^^ up
    else if (presses & (1<<8) ){ // EK-TM4C1294XL button 2 pressed
        fifo_put ('W'); //This is working
        isPutFlag = 0;
    }

    //VVVVVVVVVVVVV down
    else if (presses & (1<<7) ){ // EK-TM4C1294XL button 2 pressed
        fifo_put ('S'); //This is working
        isPutFlag = 0;
    }
